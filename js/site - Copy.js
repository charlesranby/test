...
var where_activity_chart = dc.rowChart("#where-activity");
var where_region_chart = dc.rowChart("#where-region");
...
cf.where_activity = cf.dimension(function (d) {
		return d.Province;
	});
...
var where_activity = cf.where_activity.group();
var all = cf.groupAll();

who_by_chart.width(300).height(220)
.dimension(cf.who_by)
.group(who_by)
.ordering(function (d) {
	return -d.who_by
...
who_to_chart.width(300).height(220)
.dimension(cf.who_to)
.group(who_to)
.colors([
...
what_activity_chart.width(300).height(220)
...
what_items_chart.width(300).height(220)
...
where_region_chart.width(300).height(220)
...
where_activity_chart.width(300).height(220)
.dimension(cf.where_activity)
.group(where_activity)
.colors([
...
activity_chart.width(300).height(220)
...
country_chart.width(300).height(220)
...
.projection(d3.geo.mercator().center([125.5, 14.5]).scale(4500))
.title(function (d) {
	return d.key;
});
